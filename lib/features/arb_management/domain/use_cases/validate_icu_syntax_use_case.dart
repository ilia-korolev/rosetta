...
class ValidateIcuSyntaxUseCase {
  ...
  void _validatePluralSyntax(
    ...
    if (match == null) {
      errors.add(ValidationError(
        ...
        entryKey: entry.key,
        suggestion: 'Use format: {count, plural, =0{no items} =1{1 item} other{# items}}',
      ));
      return;
    }
    ...
    final validSelectors = {'=0', '=1', 'zero', 'one', 'two', 'few', 'many', 'other'};
    for (final icuCase in cases) {
      if (!validSelectors.contains(icuCase.selector) && !icuCase.selector.startsWith('=')) {
        warnings.add(ValidationWarning(
          message: 'Unknown plural selector "${icuCase.selector}"',
          code: 'UNKNOWN_PLURAL_SELECTOR',
          entryKey: entry.key,
          suggestion: 'Use standard selectors: =0, =1, zero, one, few, many, other',
        ));
      }
    }
    ...
    final duplicates = selectors.where((s) => selectors.where((s2) => s2 == s).length > 1).toSet();
  ...
  void _validateSelectSyntax(
    ...
    if (match == null) {
      errors.add(ValidationError(
        ...
        entryKey: entry.key,
        suggestion: 'Use format: {gender, select, male{he} female{she} other{they}}',
      ));
      return;
    }
    ...
    for (final icuCase in cases) {
      if (!selectorPattern.hasMatch(icuCase.selector)) {
        warnings.add(ValidationWarning(
          message: 'Invalid select selector "${icuCase.selector}". Use alphanumeric characters only.',
  ...
  void _validateCompoundSyntax(
    ...
    warnings.add(ValidationWarning(
      ...
      entryKey: entry.key,
      suggestion: 'Ensure nested structures are properly balanced and complete',
    ));
  }
  ...
  void _validatePlaceholderSyntax(
    ...
    final messageePlaceholders = placeholderPattern.allMatches(message)
        .map((m) => m.group(1)!)
        .toSet();
    ...
    for (final placeholder in messageePlaceholders) {
      if (!metadataPlaceholders.contains(placeholder)) {
        warnings.add(ValidationWarning(
          message: 'Placeholder "{$placeholder}" used in message but not defined in metadata',
    ...
    for (final placeholder in metadataPlaceholders) {
      if (!messageePlaceholders.contains(placeholder)) {
        suggestions.add(ValidationSuggestion(
          message: 'Placeholder "$placeholder" defined in metadata but not used in message',
    ...
    if (entry.placeholders != null) {
      for (final placeholder in entry.placeholders!.values) {
        ...
        if (placeholder.format != null) {
          _validateFormatCompatibility(placeholder, errors, warnings, entry.key);
        }
  ...
  void _validateGeneralIcuSyntax(
    ...
    if (braceCount > 0) {
      errors.add(ValidationError(
        message: 'Unmatched opening brace(s). Missing $braceCount closing brace(s)',
    ...
    switch (placeholder.type) {
      case PlaceholderType.dateTime:
        final validDateFormats = [
          'yMd', 'yMMMMd', 'yMMMd', 'yMMMEd', 'yMMMMEEEEd', 'jm', 'Hm', 'Hms',
          'yMMMEd', 'yQQQ', 'EEEE', 'MMMM', 'MMM', 'LLLL', 'LLL'
        ];
        if (!validDateFormats.contains(format)) {
          warnings.add(ValidationWarning(
            message: 'Unknown date format "$format" for placeholder "${placeholder.name}"',
            code: 'UNKNOWN_DATE_FORMAT',
            entryKey: entryKey,
            suggestion: 'Use standard date formats like yMd, yMMMMd, jm, etc.',
          ));
        }
        break;
      ...
      case PlaceholderType.double:
        final validNumberFormats = [
          'compact', 'compactCurrency', 'compactSimpleCurrency', 'compactLong',
          'currency', 'decimalPattern', 'decimalPatternDigits', 'decimalPercentPattern',
          'percentPattern', 'scientificPattern', 'simpleCurrency'
        ];
        if (!validNumberFormats.contains(format)) {
          warnings.add(ValidationWarning(
            message: 'Unknown number format "$format" for placeholder "${placeholder.name}"',
            code: 'UNKNOWN_NUMBER_FORMAT',
            entryKey: entryKey,
            suggestion: 'Use standard number formats like compact, currency, percentPattern, etc.',
          ));
        }
        break;
      ...
      case PlaceholderType.string:
        warnings.add(ValidationWarning(
          message: 'Format "$format" specified for String placeholder "${placeholder.name}"',
          code: 'UNNECESSARY_STRING_FORMAT',
          entryKey: entryKey,
          suggestion: 'String placeholders typically do not need format specifications',
        ));
        break;
    }
  }
    ...
    if (placeholder.format != null) {
      ...
      for (final param in params.keys) {
        if (!validParams.contains(param)) {
          warnings.add(ValidationWarning(
            message: 'Unknown optional parameter "$param" for format "${placeholder.format}"',
...
class IcuValidationResult extends ValidationResult {
  ...
  List<ValidationError> get icuSyntaxErrors {
    return errors.where((e) => e.code.startsWith('INVALID_') || 
                                e.code.startsWith('MISSING_') ||
                                e.code.startsWith('UNMATCHED_')).toList();
  }
